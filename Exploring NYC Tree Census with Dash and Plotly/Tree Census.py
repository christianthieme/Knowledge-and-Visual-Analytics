...
app.layout = html.Div([
    ...
    html.Div([
        html.H4("How many species to display?:"),
    ...
    html.Div([
        ...
        html.Div([
        html.H4("Steward Project Statistics - Do Stewards Make a Difference in Tree Health Overall?:"),
        ...
        dcc.Graph(id = 'grouped-graphic', figure = px.bar(steward_percent_df, x = 'health', y = 'percent', color = 'steward', barmode = 'group', text = round(steward_percent_df['percent'],2),
        labels={
                 "percent": "Percent", 
                 "steward": "Steward",
                 "health": "Tree Health"
               }))],  style = {'padding-left': '30px','display':'inline-block','width':'48%'}),

    html.Hr(),
    html.Div([
        html.H2("Tree Health Drill Down"),
        html.P("The selectors below allow you to look at boroughs and dive into the details of tree species.")], style = {'width':'96%'}),
    ...
    html.Div([
        ...
        dcc.Dropdown(id = 'species', options = [{'label':i, 'value':i} for i in df['spc_common'].unique()], multi = True, value = ['American Beech', 'American Elm', 'American Hophornbeam'])], 
        style = {'width':'48%', 'display':'inline-block', 'padding-left': '15px'}),
    ...
    html.Div(dcc.Graph(id = 'species_percent'), style = {'width':'96%'})
    ])
...
def update_all_bar(top_x):
    ...
    return px.bar(fd, x = 'Species', y = 'count_tree_id',  color = 'boroname', title = "Top Tree Species by Borough", 
        labels={
...
def update_graph(val_boro, val_species):
        filtered_df = df[(df['boroname'].isin(val_boro)) & (df['spc_common'].isin(val_species))]
...
def update_area_graph(val_boro, val_species):
        ...
        fig = px.treemap(grouped_again, path = [px.Constant('New York City'), 'boroname', 'health'], values = 'percent', color = 'boroname', hover_name = 'boroname', title = "Tree Health Proportion by Borough", 
        labels={
                 "percent": "Percent:"
               })
...
@app.callback(Output('species_percent', 'figure'), [Input('boro', 'value'), Input('species', 'value')])
def update_species_percent_graph(val_boro, val_species):
    filtered_df = df[(df['boroname'].isin(val_boro)) & (df['spc_common'].isin(val_species))]
    steward_df = filtered_df.groupby(['steward', 'health']).sum()
    steward_percent_df = steward_df.groupby(level=0).apply(lambda x:1 * x / float(x.sum())).reset_index().rename(columns = {'count_tree_id':'percent'})
    steward_percent_df['steward'] = np.where(steward_percent_df['steward'] =='None','0',steward_percent_df['steward'])
    steward_percent_df.sort_values(by = 'steward', inplace = True)
    return px.bar(steward_percent_df, x = 'health', y = 'percent', color = 'steward', barmode = 'group', text = round(steward_percent_df['percent'],2), title = "Steward Project Comparative Percentages - Do Stewards Make a Difference in Tree Health for the Selected Borough(s) and Species?", 
        labels={
                 "percent": "Percent", 
                 "steward": "Steward",
                 "health": "Tree Health"
